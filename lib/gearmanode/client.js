...
var Client = exports.Client = function(options) {
    ...
    Client.logger.info('client initialized with %d job server(s)', this.jobServers.length);
    Client.logger.debug('load balancing: strategy=%s, recoverTime=%d[ms]', clientOptions.loadBalancing, clientOptions.recoverTime);
};
...
Client.prototype.submitJob = function(name, payload, options) {
    ...
    var jsSendCallback = function(err) {
        if (err instanceof Error) {
            Client.logger.warn('failed to submit job, server=%s, error:', jobServer.getUid(), err);
            tryToSend();
        } else {
            ...
            Client.logger.debug('job submited, name=%s, unique=%s', job.name, job.unique);
        }
    };
...
Client.prototype._response = function (jobServer, packetType, parsedPacket) { // #unit: not needed
    ...
    Client.logger.debug('response for client: type=%s, uid=%s', packetCode, job.getUid());
    ...
    case protocol.PACKET_TYPES.STATUS_RES:
        ...
        Client.logger.debug('job status, handle=%s, known=%d, running=%d, num=%d, den=%d',
    ...
    case protocol.PACKET_TYPES.WORK_STATUS:
        ...
        Client.logger.debug('job status, handle=%s, num=%d, den=%d',
