    ...
    if (this.connected) {
        ...
        eventNames.forEach(function (name) {
            self.socket.on(name, function() {
                JobServer.logger.warn('unhandled event, name=%s', name);
            });
        });
        ...
        this.socket.on('connect', function () {
            ...
            JobServer.logger.debug('connection established, uid=%s', self.getUid());
            callback();
        });

        // emitted when an error occurs
        this.socket.on('error', function (err) {
            JobServer.logger.error('socket error:', err);
...
// }
            JobServer.logger.debug('received packet, len=%d', chunk.length);
            JobServer.logger.debug('received packet: %s', common.bufferAsHex(chunk));
...
// }
        ...
        this.socket.on('close', function (had_error) {
            if (had_error) { // if the socket was closed due to a transmission error
                JobServer.logger.warn('connection closed due to an transmission error, uid=%s', self.getUid());
                // inform load balancer that this server is invalid
                if (self.clientOrWorker._type === 'Client') {
                    self.clientOrWorker.loadBalancer.badOne(self.clientOrWorker._getJobServerIndexByUid(self.getUid()));
                }
        ...
        this.socket.on('end', function (err) {
            JobServer.logger.warn('connection terminated, uid=%s', self.getUid());
            // inform load balancer that this server is invalid
            if (self.clientOrWorker._type === 'Client') {
                self.clientOrWorker.loadBalancer.badOne(self.clientOrWorker._getJobServerIndexByUid(self.getUid()));
            }
...
JobServer.prototype.disconnect = function (err) {
    ...
    JobServer.logger.debug('connection closed, uid=%s', this.getUid());
};
...
JobServer.prototype.send = function (data, callback) {
    ...
    sendCb = function () {
        ...
        JobServer.logger.debug('packet sent, type=%s, len=%d', protocol.PACKET_CODES[data.readUInt32BE(4)], data.length);
    ...
    if (this.connected) {
        ...
        JobServer.logger.debug('unconnected job server, uid=%s', this.getUid());
...
JobServer.prototype._processData = function (chunk) {
    if (!this.connected) {
        JobServer.logger.warn('trying to process data from disconnected job server (disconnect before all packet received?)');
        return;
    }
    ...
    if (chunk.length < responseLength) {
          JobServer.logger.debug('segmented packet found, responseSize=%d, packetLen=%d', responseLength, chunk.length);
    ...
    if (chunk.length > responseLength) {
        JobServer.logger.debug('joined packet found, responseSize=%d, packetLen=%d', responseLength, chunk.length);
    ...
    case protocol.PACKET_TYPES.ERROR:
        JobServer.logger.warn(job server error, uid=%s, code=%s, msg=%s',
    ...
    case protocol.PACKET_TYPES.JOB_CREATED:
        ...
        if (this.jobsWaiting4Created.length === 0) {
            JobServer.logger.debug('no more a job waiting 4 state JOB_CREATED');
        }
    ...
    default:
        JobServer.logger.warn('unknown packet, type=%d', packetType);
