...
LBStrategy.prototype._searchNextGood = function (badIdx) {
    ...
    LBStrategy.logger.warn('identified bad node, idx=%d, age=%d [ms]', badIdx, (this.badNodes[badIdx] - new Date()));
    ...
    for (var i = 0; i < this.nodeCount; i ++) {
        ...
        if (this.badNodes.hasOwnProperty(candidate)) {
            ...
            if ((new Date() - failureTime) > this.recoverTime) {
              ...
              LBStrategy.logger.debug('node timeout recovery, idx=%d', candidate);
              this.goodOne(candidate)
            }
        } else {
            LBStrategy.logger.debug('found good node, idx=%d', candidate);
            return candidate;
        }
    }
    ...
    if (timeoutCandidate !== null) {
      LBStrategy.logger.debug('good node not found, delivering timeouted one, idx=%d', timeoutCandidate);
      return timeoutCandidate;
    }

    LBStrategy.logger.error('all nodes invalid, no candidate more');
    return null;
};
