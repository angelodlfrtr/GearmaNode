...
var Worker = exports.Worker = function(options) {
    ...
    Worker.logger.info('worker initialized with %d job server(s)', this.jobServers.length);
};
...
Worker.prototype.addFunction = function(name, callback, options) {
    ...
    for (var i = 0; i < this.jobServers.length; i ++) { // TODO iterate only servers without any previous error
        ...
        Worker.logger.info('registered function name=%s on server uid=%s', name, jobServer.getUid());
    }
};
...
Worker.prototype.removeFunction = function(name) {
    ...
    for (var i = 0; i < this.jobServers.length; i ++) { // TODO iterate only servers without any previous error
        ...
        Worker.logger.debug('unregistered function name=%s on server uid=%s', name, jobServer.getUid());
    }
};
...
Worker.prototype.resetAbilities = function() {
    ...
    Worker.logger.debug('RESET_ABILITIES on all servers');
};
...
Worker.prototype.setWorkerId = function(workerId) {
    ...
    Worker.logger.debug('SET_CLIENT_ID on all servers, workerId=%s', this.workerId);
};
...
Worker.prototype._response = function (jobServer, packetType, parsedPacket) { // #unit: not needed
    ...
    case protocol.PACKET_TYPES.JOB_ASSIGN_UNIQ:
        ...
        Worker.logger.debug('function finished, name=%s, result=' + fnRslt, job.name);
        break;
...
Worker.prototype._getFunction = function (name) { // #unit: not needed
    if (this.functions.hasOwnProperty(name)) {
        ...
        Worker.logger.error('function not found in locale register, name=%s', name);
...
common.mixin({
    ...
    sendWorkData: function (data) {
        ...
        Worker.logger.debug('work data, handle=%s, data=%s', this.handle, data.toString());
    ...
    reportError: function () {
        Worker.logger.warn('work failed, handle=%s', this.handle);
    ...
    reportException: function (data) {
        ...
        Worker.logger.warn('work failed with exception, handle=%s, exception=%s', this.handle, data.toString());
    ...
    reportWarning: function (data) {
        data = data || '[no more details provided by worker]';
        var jobServer = this.clientOrWorker._getJobServerByUid(this.jobServerUid);
        Worker.logger.warn('worker warning, handle=%s, msg=%s', this.handle, data.toString());
